...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE tbl_slider_ruta SET heading=?, content=?, position=? WHERE id=?");
        	...
        	$statement = $pdo->prepare("UPDATE tbl_slider_ruta SET photo=?, heading=?, content=?, position=? WHERE id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_slider_ruta WHERE id=?");
