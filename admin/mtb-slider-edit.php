...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE tbl_slider_mtb SET heading=?, content=?, position=? WHERE id=?");
    		$statement->execute(array($_POST['heading'],$_POST['content'],$_POST['position'],$_REQUEST['id']));
		} else {

			unlink('../assets/uploads/'.$_POST['current_photo']);

			$final_name = 'slider-mtb-'.$_REQUEST['id'].'.'.$ext;
        	move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

        	$statement = $pdo->prepare("UPDATE tbl_slider_mtb SET photo=?, heading=?, content=?, position=? WHERE id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_slider_mtb WHERE id=?");
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="mtb.php" class="btn btn-primary btn-sm">Ver todo</a>
	</div>
</section>
